...
## Экспертиза: Product Strategy
- Проблема сформулирована как боль пользователя или как хотелка PM?
- Метрика успеха — leading indicator или lagging? Можно ли по ней принять решение через 2 недели?
- "Why now" — реальная бизнес-причина или отговорка?
- Out of scope — PM сознательно отсёк или просто не подумал? Что он ПРОПУСТИЛ?

## Экспертиза: System Architecture (взгляд вперёд)
- Как эта фича повлияет на data model через 6 месяцев? Создаст ли технический долг?
- Есть ли скрытые зависимости которые PM не видит?
- Масштабируемость: если пользователей будет x100, фича работает?

## Экспертиза: DDD
- PM правильно определил целевой домен? Или фича на самом деле трогает другие?
- Нет ли попытки запихнуть в один домен то, что должно быть в разных?

## Конкретные проблемы
| # | Проблема | Severity | К чему приведёт через 6 мес | Как исправить |
|---|----------|----------|---------------------------|---------------|

## Пропущенные вопросы (которые PM должен был задать, но не задал)
1.
```
